...
export default function SyncDashboard() {
  ...
  // Initial load SAP counts when configs are first loaded
  useEffect(() => {
  ...
  useEffect(() => {
    fetchStatus();
    const interval = setInterval(fetchStatus, 10000); // Poll DB every 10 seconds
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl font-semibold transition-all text-sm ${
                  ...
                  : 'bg-evolver-bg-obsidian border border-white/10 hover:border-evolver-viridian/50 text-white'
              }`}
            >
            ...
            >
              ...
              {syncing ? 'Extraction in Progress...' : 'Full Overlay Sync'}
            </button>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="glass-panel p-6 rounded-2xl border-l-4 border-l-evolver-blue flex justify-between items-center">
            <div>
              <h3 className="text-slate-400 font-semibold mb-2">Cached Tables (DD02L)</h3>
              <p className="text-4xl font-mono text-white">{(stats.dd02l || 0).toLocaleString()}</p>
            </div>
            <div className="text-right">
              <span className="text-xs text-slate-500 block uppercase tracking-wider mb-1">Status</span>
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                <CheckCircle2 className="w-3.5 h-3.5" /> Replicated
              </span>
            </div>
          </div>
          <div className="glass-panel p-6 rounded-2xl border-l-4 border-l-evolver-viridian flex justify-between items-center">
            <div>
              <h3 className="text-slate-400 font-semibold mb-2">Cached Relationships (DD08L)</h3>
              <p className="text-4xl font-mono text-white">{(stats.dd08l || 0).toLocaleString()}</p>
            </div>
            <div className="text-right">
              <span className="text-xs text-slate-500 block uppercase tracking-wider mb-1">Status</span>
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                            ...
                            <table className="w-full text-left">
                                <thead className="border-b border-white/5 text-xs text-slate-400">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 font-medium text-right">Local Records</th>
                                        <th className="px-6 py-3 font-medium text-right">Live SAP Count</th>
                                        <th className="px-6 py-3 font-medium text-right">Strategy</th>
                                    </tr>
                                </thead>
                                    ...
                                    {items.map((config) => (
                                        <tr key={config.id} className="hover:bg-white/5 transition-colors group">
                                            ...
                                            <td className="px-6 py-4 text-right font-mono text-sm text-slate-400">
                                                {typeof sapCounts[config.id] === 'number' ? (sapCounts[config.id] as number).toLocaleString() : sapCounts[config.id] || "..."}
                                            </td>
                    ...
                    <td className="p-4 font-mono text-sm">
                      {log.status === 'RUNNING' ? (
                        <div className="flex items-center gap-2 text-blue-400">
                          ...
                          {log.records.toLocaleString()} <span className="text-xs text-blue-500/70">extracted...</span>
                        </div>
                      ) : (
