...
export default function LogicalDBPage() {
  ...
  return (
    <div className="relative w-full h-full flex flex-col flex-1 bg-slate-50 dark:bg-evolver-bg-dark transition-colors duration-500">
      {/* Top Navigation & Filters */}
      <div className="absolute top-6 left-1/2 -translate-x-1/2 z-10 w-full max-w-5xl px-6 flex flex-col items-center gap-4">
        <div className="bg-white/80 dark:bg-evolver-bg-obsidian/80 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-2xl p-2 shadow-2xl flex items-center justify-center">
            <button 
                onClick={() => setViewMode('3d')}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${viewMode === '3d' ? 'bg-evolver-viridian text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
            >
                3D Macro Map
            </button>
            <button 
                onClick={() => setViewMode('2d')}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${viewMode === '2d' ? 'bg-blue-600 text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
            >
                2D ER Diagram
            </button>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
            <button
                ...
                className={`px-4 py-1.5 rounded-full text-xs font-semibold border transition-all shadow-lg ${
                    activeModule === 'All' 
                    ? 'bg-slate-900 dark:bg-white text-white dark:text-black border-slate-900 dark:border-white' 
                    : 'bg-slate-100 dark:bg-black/40 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-white/10 hover:border-slate-350 dark:hover:border-white/30'
                }`}
            >
                All Modules
            </button>
