...
async function run() {
    try {
        console.log("Fetching one line item with supplier context...");
        const res = await odataQuery("API_GLACCOUNTLINEITEM/GLAccountLineItem", "&$top=1&$filter=Supplier ne '' and CompanyCode eq '1710'");
        const results = res.d?.results || [];
        if (results.length > 0) {
            console.log("Success! Raw properties of GLAccountLineItem for Supplier:");
            console.log(JSON.stringify(results[0], null, 2));
        } else {
            console.log("No items found with Supplier not empty");
        }
    } catch (e) {
