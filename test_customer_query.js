...
async function run() {
    ...
    try {
        console.log("\n--- Querying I_Customer ---");
        const res = await odataQuery(service, "I_Customer", "&$top=3");
        console.log("Success! Found:", res?.d?.results?.length);
        if (res?.d?.results) {
            res.d.results.forEach(row => {
                console.log(`- ID: ${row.Customer}, Name: ${row.CustomerName || row.FullName || row.BusinessPartnerName}`);
                console.log("Full row keys:", Object.keys(row));
            });
        }
    } catch (e) {
        console.error("I_Customer query failed:", e.message);
    }

    // 2. Try I_Customer_VH
    try {
        console.log("\n--- Querying I_Customer_VH ---");
        const res = await odataQuery(service, "I_Customer_VH", "&$top=3");
        console.log("Success! Found:", res?.d?.results?.length);
        if (res?.d?.results) {
            res.d.results.forEach(row => {
                console.log(`- ID: ${row.Customer}, Name: ${row.CustomerName || row.FullName || row.BusinessPartnerName || row.CustomerName_Text}`);
                console.log("Full row keys:", Object.keys(row));
            });
        }
    } catch (e) {
        console.error("I_Customer_VH query failed:", e.message);
    }
}
