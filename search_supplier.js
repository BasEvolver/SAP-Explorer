...
async function run() {
    try {
        console.log("Fetching all from C_NoCompanyCodeSupplier...");
        const data = await makeRequest(`${baseUrl}/C_NOCOMPANYCODESUPPLIER_CDS/C_NoCompanyCodeSupplier?$format=json&$top=100`);
        const results = data.d?.results || [];
        console.log(`Success! Found ${results.length} rows.`);
        results.forEach(row => {
            console.log(`- Supplier: ${row.Supplier}, Name: ${row.SupplierName}`);
        });
    } catch (e) {
